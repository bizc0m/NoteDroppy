...
APP_SRC="${1:-$ROOT_DIR/outputs/Note Commander.app}"
APP_DST="/Applications/Note Commander.app"
BACKUP_DIR="$HOME/Library/Application Support/Note Commander/backups"
...
if [[ -d "$APP_DST" ]]; then
  mkdir -p "$BACKUP_DIR"
  mv "$APP_DST" "$BACKUP_DIR/Note Commander.app.previous-$(date +%Y%m%d-%H%M%S).bundle-backup"
fi
