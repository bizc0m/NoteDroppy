#!/bin/zsh
# Build Note Droopy V5 — local, sans reseau, sans dependance externe.
# Usage : ./scripts/build-notedroppy-v5.sh [version] [build]
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP="$ROOT_DIR/outputs/Note Droopy V5.app"
SRC_DIR="$ROOT_DIR/work/NoteDroppyV4"
BIN="$APP/Contents/MacOS/NoteDroopyV5"
PLIST="$APP/Contents/Info.plist"
VERSION="${1:-5.0}"
BUILD="${2:-500}"
BUNDLE_ID="local.codex.notedroppy.v5"
...
<dict>
  ...
  <string>NoteDroopyV5</string>
  <key>CFBundleIdentifier</key>
  <string>$BUNDLE_ID</string>
  <key>CFBundleName</key>
  <string>Note Droopy V5</string>
  ...
  <string>Note Droopy V5 a besoin d'envoyer des evenements Apple a NotePlan et a ton navigateur (Safari/Chrome) pour recuperer le lien de la page ou de la note source lors d'une capture.</string>
</dict>
</plist>
PLIST
...
ENTITLEMENTS="$ROOT_DIR/scripts/NoteDroopyV5.entitlements"
